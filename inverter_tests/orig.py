...
class inverter_tests(rtl, eldo, thesdk):
    @property
    def _classfile(self):
    ...
    def run(self, *arg):
        ...
        if self.model == "py":
            ...
            if self.model == "vhdl":
                ...
                self.nproc = 1
                self.eldooptions = {"eps": "1e-6"}
                self.eldoparameters = {"exampleparam": "0"}
                self.eldoplotextras = ["v(IN)", "v(OUT)"]

                # Example of defining supplies (not used here because the example inverter_tests has no supplies)
    ...
    def define_io_conditions(self):
        ...
        self.iofile_bundle.Members["Z"].verilog_io_condition_append(
            cond="&& initdone"
        )
...
if __name__ == "__main__":
    ...
    from inverter_tests.controller import (
        controller as inverter_tests_controller,
    )
    import pdb
    ...
    controller = inverter_tests_controller()
    controller.Rs = rs
    # controller.reset()
    ...
    for k in range(len(duts)):
        ...
        if duts[k].model == "eldo":
            ...
            axes[0].plot(
                duts[k].IOS.Members["A_OUT"].Data[:, 0],
                duts[k].IOS.Members["A_OUT"].Data[:, 1],
                label="Input",
            )
            axes[1].plot(
                duts[k].IOS.Members["Z"].Data[:, 0],
                duts[k].IOS.Members["Z"].Data[:, 1],
                label="Output",
            )
            ...
            axes[1].stem(
                ...
                duts[k]
                .IOS.Members["Z"]
                .Data[0 + latency[k] : 11 + latency[k], 0],
            )
